...
<div class="row">
        ...
        <div class="col-sm-4 col-md-4">
		 <?php $this->load->view($this->themename.'/layout/admin/globe/sidebar/nav'); ?>
			...
			<h6>
			<?php echo $this->lang->line('pending_withdrawal_well_h6');?>
	  ...
	  <?php
		...
		// Pending Deposit failed alert.
      	if ($this->session->pending_accept_withdrawal_failed) {
      		echo '<div class="alert alert-danger alert-dismissible" role="alert">
			<a href="#" class="close" data-dismiss="alert" aria-label="close"><i class="fa fa-times-circle" aria-hidden="true"></i></a>
      			  	'.$this->lang->line('pending_accept_withdrawal_failed').'
      			  </div>';
      	}
		
		// Pending Deposit success alert.
      	if ($this->session->pending_accept_withdrawal_success) {
      		echo '<div class="alert alert-success alert-dismissible" role="alert">
			<a href="#" class="close" data-dismiss="alert" aria-label="close"><i class="fa fa-times-circle" aria-hidden="true"></i></a>
      			  	'.$this->lang->line('pending_accept_withdrawal_success').'
      			  </div>';
      	}
		...
		<div class="panel panel-default panel-transaction">
			...
			} elseif ($pending[$i]->payment_type == 'Withdraw') {
			echo $pending[$i]->receiver;
			
			} elseif ($pending[$i]->payment_type == 'Card Verified') {
			echo $pending[$i]->receiver;
			
			} elseif ($pending[$i]->sender_name) {
				if (empty($pending[$i]->sender)) {
					echo ''.$pending[$i]->sender_name.' (Guest User)';
			}else {
			echo $pending[$i]->sender_name;
			}
			}?>
			</span>
			...
			echo '<p class="payment-right-amount-negative">-'.$this->site_settings->curr_symb.''.$pending[$i]->amount.'</p>';
			} elseif ($pending[$i]->payment_type == 'request') {
			echo '<p class="payment-right-amount-positive">+'.$this->site_settings->curr_symb.''.$pending[$i]->total.'<p>';
			} elseif ($pending[$i]->sender_name) {
			echo '<p class="payment-right-amount-negative">-$'.$pending[$i]->total.'</p>';
			}?>
			</div>
			</br>
			<!-- Payment type -->
			...
			echo '<p class="payment-space-tabs">'.$pending[$i]->payment_type.'</p>';
			
			} elseif ($pending[$i]->sender_name) {
			if ($pending[$i]->shipping == '1') {
			echo '<p class="payment-space-tabs">Product Paid</p>';
				} else {
					echo '<p class="payment-space-tabs">Payment '.$pending[$i]->payment_type.' </br>';
				}
			}?>
			...
			echo $pending[$i]->payment_method.' Account';
			}?>
					 </strong>
					 <!-- Receiver name -->
					 <p><?php
			...
			<!-- refund -->
			 <form role="form" method="post" action="'.site_url('admin/pending/deposit').'">
					 ...
					 <button class="panel-refund-button" type="submit" name="pending-deposit">Accept Request</button>
					 </form>
			';
			...
			<!-- Payment Sent -->
			<div class="row">
			<div class="col-md-6"><strong>'.$pending[$i]->status.'</strong> Amount  '.$pending[$i]->receiver_name.'</div><div class="col-md-6"><span class="pull-right">'.$this->site_settings->curr_symb.''.$pending[$i]->total.'</span></div>
			...
			<div class="col-md-6"><strong>Total</strong></div><div class="col-md-6"><span class="pull-right"><strong>'.$this->site_settings->curr_symb.''.$pending[$i]->amount.'</strong></span></div>
			 ...
			 <!-- Accept form -->
					 <form role="form" method="post" action="'.site_url('admin/pending/withdraw').'">
					 <input type="hidden" name="transaction_id" value="'.$pending[$i]->txn_id.'">
					 <input type="hidden" name="'. $this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">
					 <button class="panel-accept-button" type="submit" name="accept-withdraw">Accept Request</button>
					 </form>
					 ...
					 <input type="hidden" name="transaction_id" value="'.$pending[$i]->txn_id.'">
					 <input type="hidden" name="'. $this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">
					 <button class="panel-refund-button" type="submit" name="cancel-withdraw">Cancel Request</button>
					 </form>
			';
			}?>
					 </span>
