	 ...
	 <!-- Cards section -->
		  ...
		  <form role="form" method="post" action="<?php echo site_url('business/wallet/card');?>">
        ...
        <button class="btn btn btn-primary active" type="submit" name="verify-card"><?php echo $this->lang->line('wallet_card_text_button_verify'); ?></button>
		 ...
		 <!-- form default card -->
		 <form role="form" method="post" action="<?php echo site_url('business/wallet_add/carddefault');?>">
		 <input type="hidden" name="id" value="<?php echo $card->id;?>">
		  <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
		...
		<div class="col-xs-6 col-md-6">
		<!-- form remove card -->
		<form role="form" method="post" action="<?php echo site_url('business/wallet_add/cardremove');?>">
		 <input type="hidden" name="id" value="<?php echo $card->id;?>">
		  <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
